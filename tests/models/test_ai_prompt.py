...
class TestAiPromptCliParity:
    """The AI usage prompt must cover the full CLI surface.

    Adding a command without documenting it fails this test — the prompt
    claims to "reflect the current feature set", so that claim is verified.
    """

    @staticmethod
    def _walk_commands(group: click.Group, prefix: tuple[str, ...] = ()) -> list[str]:
        """Enumerate every command path of a click group (e.g., 'budjira worklog update')."""
        paths = []
        for name, command in sorted(group.commands.items()):
            path = (*prefix, name)
            paths.append("budjira " + " ".join(path))
            if isinstance(command, click.Group):
                paths.extend(TestAiPromptCliParity._walk_commands(command, path))
        return paths

    def test_every_cli_command_is_documented(self) -> None:
        """Every registered command path appears in the default template."""
        import typer.main
        ...
        content = "\n".join(section.content for section in get_default_ai_prompt_template().sections if section.enabled)

        missing = [path for path in self._walk_commands(root) if path not in content]
        assert not missing, "CLI commands missing from the AI usage prompt: " + ", ".join(missing)

    def test_no_nested_single_command_groups(self) -> None:
        """Single-command sub-apps must not be registered as groups (breaks documented syntax).

        Regression test: `budjira search "JQL"` failed with 'No such command' when
        search.app (one command) was registered via add_typer, forcing
        `budjira search search "JQL"`. Single-command modules are registered
        directly with app.command(), like `show` and `attach`.
        """
        import typer.main
        from budjira.cli.main import app
        ...
        for name, command in root.commands.items():
            if isinstance(command, click.Group):
                assert command.commands, f"command group '{name}' has no subcommands"
                for sub_name in command.commands:
                    assert sub_name != name, (
